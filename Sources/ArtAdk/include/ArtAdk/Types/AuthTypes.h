...
/// Credential hook, called again on every authentication. Takes precedence
/// over `-[Adk setCredentials:]` and `autoLoadCredsFromJSON`.
@property(nonatomic, copy, nullable) CredentialStore *_Nonnull (^getCredentials)
    (void);
/// Folder containing `adk-services.json`. When nil, the file is looked up
/// in the app bundle.
@property(nonatomic, copy, nullable) NSString *root;
/// Load credentials from `adk-services.json` on `connect`. Defaults to NO.
@property(nonatomic, assign) BOOL autoLoadCredsFromJSON;
...
@interface CredentialStore : NSObject <NSCopying>
...
@interface AuthenticationConfig : NSObject <NSCopying>
...
@interface AuthData : NSObject <NSCopying>
