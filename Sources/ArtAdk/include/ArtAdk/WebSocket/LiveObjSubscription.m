...
- (void)executeServerMerge:(NSArray<CRDTOperation *> *)ops {
    ...
    __weak typeof(self) weakSelf = self;
    [self pushArray:@"merge"
               data:serializedOps
         completion:^(NSError *error) {
           if (error) {
               ARTLogWarn(@"Shared-object merge failed on %@: %@",
                          weakSelf.channelConfig.channelName,
...
- (void)handleMessage:(NSString *)event payload:(NSDictionary *)payload {
    ...
    // Presence: emit the parsed `data`; the raw JSON text would never match
    // the dictionary handler in `fetchPresence`.
    if ([event isEqualToString:@"art_presence"]) {
        id content = payload[@"data"];
        if (content) {
            id parsed = [content isKindOfClass:[NSString class]]
                            ? [ARTJSON parse:(NSString *)content]
                            : nil;
            [self.emitter emit:@"art_presence" data:parsed ?: content];
        }
        return;
    }

    // Only `update` frames carry remote operations.
