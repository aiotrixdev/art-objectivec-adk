...
- (void)reconnect {
    [self validateInterception:^(NSError *error) {
      if (error) {
          ARTLogWarn(@"Interceptor %@ could not be restored: %@",
                     self.interceptorName, error.localizedDescription);
      }
    }];
}
...
- (NSDictionary *)createResponse:(NSDictionary *)config
    ...
    response[@"content"] = [ARTJSON stringify:content error:nil] ?: @"";
...
- (void)execute:(NSDictionary *)request {
    ...
    // Forward the agentic routing fields, so the server can correlate the
    // interceptor's resolve / reject with the original frame.
    for (NSString *k in @[
    ...
    InterceptorResolve resolve = ^(id data) {
      ...
      // Accepts a JSON object or array. An object that echoes the envelope
      // (`attempt_id` / `pipeline_id`) is unwrapped to its `data`; an array
      // is sent as it is.
      id sanitized = nil;
      if ([data isKindOfClass:[NSDictionary class]]) {
          ...
          ARTLogError(@"Invalid data: Expected a JSON object or array of "
                      @"objects.");
          return;
      }
...
- (void)acknowledge:(NSDictionary *)request {
    ...
    if (dataVal) {
        response[@"content"] = [ARTJSON stringify:dataVal error:nil] ?: @"";
    }
...
- (void)handleMessage:(NSString *)channel data:(NSDictionary *)data {
    ...
    if ([dataField isKindOfClass:[NSString class]]) {
        id parsed = [ARTJSON parse:(NSString *)dataField];
        if (parsed) {
            mutable[@"data"] = parsed;
        }
    }
...
- (void)sendJSON:(NSDictionary *)dict {
    NSError *error = nil;
    NSString *frame = [ARTJSON stringify:dict error:&error];
    if (!frame) {
        ARTLogError(@"Interceptor response is not JSON-serializable: %@",
